# Legacy: PointBot (Python / Selenium)

> **This is the original, now-discontinued PointBot implementation**, preserved
> for reference. It is **not** built, tested, or deployed by the modern
> TypeScript monorepo at the repository root. See
> [`docs/migration-from-pointup.md`](../../docs/migration-from-pointup.md) for
> how and why the product was modernized.
>
> The real value here is the **per-provider scraping logic** (Marriott,
> Southwest, United, Hyatt, Delta, American, MGM, Frontier bots under
> `src/point_bot/`). In the modern architecture these map onto the hexagonal
> `TravelProviderGateway` port — a future real-scraper adapter can port this
> logic behind that port without the rest of the system knowing scraping is
> involved. The gateway is currently backed by `SimulatedTravelProviderGateway`.
